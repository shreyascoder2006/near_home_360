...
import type { Alert, FeedEvent, Recommendation, RequestType, ServiceRequest, SimState, Staff, Severity } from "./types";
...
export function tick(state: SimState, model: ResortModel, dtMin: number) {
  ...
  for (const r of rooms) {
    const rate = r.conditioned ? 1.55 : 0.18;
  ...
  if (Math.floor(state.t / 30) !== Math.floor((state.t - dtMin) / 30)) {
